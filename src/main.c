...
static int process_command(t_cmd *cmds, t_shell *shell)
{
    ...
    int result = execute_command_main(cmds, shell);
...
static int process_command_from_input(const char *input, t_shell *shell)
{
    ...
    if (!cmds) {
        // Debug: Eğer input tamamen boş ise (sadece whitespace), exit code 0 olmalı
        int i = 0;
    ...
    // Eğer argv yok ama heredoc veya redirection varsa bu normal
    // Ayrıca argv boş array ise (örn: $EMPTY expand edildi) bu da normal
    if (cmds && !cmds->argv && !cmds->heredocs && !cmds->redirs) {
        // Sadece argv, heredoc VE redirs yoksa parse error
        free_cmds(cmds);
        return 0; // Error 
    }
    
    // Eğer argv boş array ise (hiç token yok), hiçbir şey yapma
    if (cmds && cmds->argv && !cmds->argv[0] && !cmds->heredocs && !cmds->redirs) {  // +
        // Boş komut - hiçbir şey yapma, exit code 0
        shell->last_exit = 0;
        free_cmds(cmds);
        return 0; // Başarılı (hiçbir şey yapılmadı)
    }
    ...
    // Çift minishell kontrolü - ./minishell ./minishell gibi durumlar için
    if (cmds && cmds->argv && cmds->argv[0] && cmds->argv[1] &&
        !ft_strcmp(cmds->argv[0], "./minishell") && !ft_strcmp(cmds->argv[1], "./minishell")) {
        ft_putstr_fd("./minishell: ./minishell: ikili dosya çalıştırılamıyor\n", 2);
        shell->last_exit = 126; // Executable format error
        free_cmds(cmds);
        return 0; // Error
    }
...
{
    // Birden fazla argüman kontrolü - bash'teki gibi hata mesajı ver
    if (argc > 1) {
        ft_putstr_fd("minishell: ", 2);
        ft_putstr_fd(argv[1], 2);
        ft_putstr_fd(": ikili dosya çalıştırılamıyor\n", 2);
        return 126; // Executable format error exit code
    }
