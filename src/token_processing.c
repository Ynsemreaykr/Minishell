...
{
    // Dinamik bellek kullanımı - daha güvenli boyut hesaplaması
    // PWD gibi uzun environment variable'lar için yeterli yer ayır 
    int base_len = end - start; // +
    int max_len = base_len < 1024 ? 4096 : base_len * 10; // Minimum 4KB, büyük input'lar için 8 kat // +
    ...
    while (i < end) {
        if (input[i] == '"') {
            ...
            // Regular character
            if (proc_len < max_len - 1) { // Buffer overflow kontrolü  // +
                processed[proc_len] = input[i];
                proc_len++;
            }
            i++;
