...
static void parse_redirections_and_heredoc(t_cmd *cmd)
{
    ...
    while (cmd->argv && cmd->argv[i]) 
    {
        ...
        {
            if (cmd->argv[i + 1]) {
                // Yeni heredoc oluştur - önce delimiter'ı kopyala
                char *original_delimiter = ft_strdup(cmd->argv[i + 1]);
                char *cleaned_delimiter = ft_strdup(cmd->argv[i + 1]);
                int quote_type = detect_quote_type_and_clean_delimiter(&cleaned_delimiter);
                t_heredoc *heredoc = create_heredoc(original_delimiter, cleaned_delimiter, quote_type);
                if (heredoc) {
                    add_heredoc(cmd, heredoc);
                }
                ft_free(original_delimiter);
                ft_free(cleaned_delimiter);
            }
