...
static t_input_result get_user_input(void)
{
    ...
    // Sinyal durumunu sıfırla - SADECE normal prompt için
    // Heredoc'tan döndükten sonra sinyal durumunu korumak için
    // Bu çağrıyı koşullu yapıyoruz
    ...
    if (signal_exit_code > 0) {
        // Eğer Ctrl+C basıldıysa, boş bir input döndür
        ft_free(input);
        result.input = ft_strdup("");  // Boş string döndür
...
static int handle_input(t_input_result input_result, t_shell *shell)
{
    ...
    if (input_result.exit_code > 0) {
        ...
        // Eğer Ctrl+C ile kesilmişse, shell'in exit code'unu güncelle
        printf("bence girmiyor\n");
        if (input_result.exit_code == 130) {
            shell->last_exit = 130;
            return 1; // Continue without processing
        }
    }
